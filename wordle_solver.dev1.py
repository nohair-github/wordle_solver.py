...
for i in range(6):
    ...
    print("value of 'wrong' before calling unmatched_letters_filter is '" + wrong + "'.")
    print()

    current_answers_list = unmatched_letters_filter(current_answers_list)

    # Tabulate and display results for current_answers_list

    current_answers_len = len(current_answers_list)
    print("len of current_answers_list is now " + str(len(current_answers_list)))
    count = len(wrong)
    if count > 1:
        print("After deselecting answers which match '"  + wrong + "' ,")
        print(str(current_answers_len) + " words are left from initial " + str(total_answers) )
        print()

    new_abridged_valid_guesses = unmatched_letters_filter(abridged_valid_guesses)
    
    # Tabulate and display results for valid guesses
    abridged_valid_guesses = new_abridged_valid_guesses
    abridged_valid_guesses_len = len(abridged_valid_guesses)
    count = len(wrong)
    if count > 1:
        print("After deselecting valid guesses which match '"  + wrong + "' ," + str(abridged_valid_guesses_len) + " valid guesses are left")
        print()
