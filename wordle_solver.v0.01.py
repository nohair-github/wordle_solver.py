...
# Preload current_answers_list with Wordle answer list

with open('wordle_answers.txt', 'r') as my_file:
...
with open('valid_guess_list.txt', 'r') as my_file:
...
for i in range(6):
    ...
    while True:
        ...
        # Confirm entry
            ...
            check(guess_word, valid_guesses)
    ...
    if len(correct) >= 1:
        ...
        for letter in correct:
            ...
            if count == 0:
                ...
                ANSWER="".join((ANSWER[:j],letter,ANSWER[j+1:]))
                if mode == 2:
                ...
                for j, v in enumerate(guess_word):
                    ...
                    if letter == v:
                        ...
                        if letter == ANSWER[j]:
                            print("Letter is already assigned to this position in ANSWER.")
                            print()
                        else:
                            if yesno("Is this an exact match (green square)"):
                                ...
                                print("Error line 509.")
                                sys.sxit()
                        
        # Select words which match pattern ANSWER

        print("Type of current_answers_list is now ", type(current_answers_list))

        if mode == 2:
            print("ANSWER is " + ANSWER)
            print(type(ANSWER))

        # Old: current_answers_list = re.findall(ANSWER, current_answers_list)

        temp_list = []
        for word in current_answers_list:
            # If the regex ANSWER is data
            if re.match(ANSWER, word):
                temp_list.append(word)

        print("After call of re, type of new_answer_list is now ", type(temp_list))

        # Tabulate results
        new_answers = len(temp_list)
        current_answers = len(current_answers_list)
        print("After matching for answers which contain " + correct + " in the pattern " + ANSWER)
        print(str(new_answers) + " words are left from initial " + str(current_answers) )
    ...
    for letter in partial:
        ...
        if mode >= 1:
            ...
            print("After selecting all current answers containing " + letter + ",")
            print(str(temp_answers) + " words are left")
        ...
        if count == 0:
            ...
            for j, v in enumerate(guess_word):
                ...
                if letter == v:
                    if mode ==2:
                        print(letter + " occurs in position " + str(j) + ".")
                    if ANSWER[j] == letter:
                        print("letter " + letter + " already in ANSWER") 
                    else:
                    ...
                    if mode >= 1:
                        temp_answers = len(temp_list)
                        print("After selecting for those answers not containing " + letter + " at index " + str(j) +",")
                        print(str(temp_answers) + " words are left.")
            ...
            # Tabulate resulte for letter in all positions of guess
            current_answers = len(current_answers_list)
            if mode >= 1:
    ...
    # Tabulate net results from Step 2
    ...
    # Determine wrong letters in guess_word after eliminating correct and partial

    matched_letters = correct + partial
    # Remove duplicate letters
    matched_letters = "".join(set(matched_letters))
    print("Previously evaluated letters are " + matched_letters)

    # Select letters not matched (black squares)
    wrong = guess_word
    for char in matched_letters:
        wrong = wrong.replace(char,'')
